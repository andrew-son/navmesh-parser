...
namespace nav_mesh {
	nav_file::nav_file( std::string nav_mesh_file ) {

		load( nav_mesh_file );
	}

	void nav_file::load( std::string nav_mesh_file ) {
		if ( !m_pather )
	...
	std::vector< nav_mesh::vec3_t > nav_file::find_path( nav_mesh::vec3_t from, nav_mesh::vec3_t to ) {
		...
		std::vector< nav_mesh::vec3_t > path = { };
		for ( std::size_t i = 0; i < path_area_ids.size( ); i++ ) {
			for ( auto& area : m_areas ) {
				if ( area.get_id( ) == std::intptr_t( path_area_ids[ i ] ) ) {
	...
	nav_area& nav_file::get_area_by_position( nav_mesh::vec3_t position ) {
		for ( auto& area : m_areas ) {
